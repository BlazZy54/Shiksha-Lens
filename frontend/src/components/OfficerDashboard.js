...
const OfficerDashboard = () => {
  ...
  const exportToCSV = (silent = false) => {
    const csvContent = [
  ...
  const downloadBackendReport = async () => {
    ...
    try {
      ...
      const response = await apiClient.downloadGovReport({ from, to });
      if (!response) {
        showMessage('warning', 'Backend report unavailable. Generated CSV fallback.');
        exportToCSV(true);
        return;
      }
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `gov-attendance-${selectedMonth}.xlsx`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
      ...
      console.error('Download gov report failed', error);
      showMessage('error', 'Failed to download backend report. Generated CSV fallback.');
      exportToCSV(true);
    } finally {
            ...
            <div className="flex space-x-3">
              ...
              >
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m0 6h4a2 2 0 002-2V7a2 2 0 00-2-2h-3l-2-2H9L7 5H4a2 2 0 00-2 2v11a2 2 0 002 2h4" />
                </svg>
                {isDownloading ? 'Downloading…' : 'Download Excel (API)'}
              </button>
              <button
                onClick={exportToCSV}
